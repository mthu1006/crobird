...
public class MainActivity extends AppCompatActivity {
    ...
    private void initCro(){
        ...
        for(int i=0; i<100; i++){
            ...
            user.setPassword(user.getName().toLowerCase().replace(" ",""));
            ...
            for(int r = 0; r<count; r++) {
                ...
                review.setId(String.valueOf(r)+String.valueOf(i));
                review.setBirdId(String.valueOf(i));
                review.setUsername(firstNames[Utils.randomWithRange(0, firstNames.length-1)]+" "+middleNames[Utils.randomWithRange(0, middleNames.length-1)]
                        +" " +lastNames[Utils.randomWithRange(0, lastNames.length-1)]);
                review.setUserId(String.valueOf(r));
                int random = Utils.randomWithRange(0, comments.length-1);
                review.setContent(comments[random]);
                review.setRating((float)Math.round(ratings[random]));
                rate+=ratings[random];
                RealmController.with(this).getRealm().copyToRealmOrUpdate(review);
                db.collection(Review.class.getSimpleName())
                        .add(review)
